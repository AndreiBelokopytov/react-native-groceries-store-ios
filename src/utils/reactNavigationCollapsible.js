...
export const setExpoStatusBarHeight = height => {
  if (Platform.OS === "android" && global.Expo) expoStatusBarHeight = height;
};
...
// let safeBounceHeight = Platform.select({ ios: 300, android: 100, web: 200 });
let safeBounceHeight = 0;
...
class _CollapsibleHeaderBackView extends Component {
  ...
  render() {
    if (Platform.OS.match(/android|web/)) return null;
    ...
    if (
      ...
      !navigation.state.params.headerY ||
...
const CollapsibleHeaderBackView = NativeNavigation.withOrientation(
  _CollapsibleHeaderBackView
);
      ...
      <Animated.View
        style={[
          options.headerStyle,
          {
            ...
            width: "100%",
            height:
              options.headerStyle.height + statusBarHeight + expoStatusBarHeight
          }
        ]}
      >
...
const collapsibleOptions = (configOptions, userOptions, navigation) => {
  ...
  if (
    !navigationParams ||
    !navigationParams.headerY ||
    navigationParams.isExtraHeader
  ) {
  ...
  const headerHeight =
    userOptions.headerStyle && userOptions.headerStyle.height
      ? userOptions.headerStyle.height
      : defaultHeaderHeight;
  if (navigationParams.headerHeight !== headerHeight) {
    const headerY = Animated.diffClamp(
      navigationParams.scrollY,
      0,
  ...
  const collapsibleOptions = {
    ...
    headerStyle: {
      ...
      opacity: Platform.select({
        ios: translateOpacity,
        android: global.Expo ? translateOpacity : 1,
        web: 1
      }),
      height: headerHeight
    },
  ...
  if (collapsibleOptions.hasTab) {
    const tabHeight =
      ...
      collapsibleOptions.tabBarOptions.style &&
    ...
    const isLandscape =
      navigationParams.isLandscape !== undefined
        ? navigationParams.isLandscape
        : isOrientationLandscape(Dimensions.get("window"));
    const tabStyle = collapsibleOptions.tabBarOptions
      ? collapsibleOptions.tabBarOptions.style
      : {};
    const paddingTop =
      getStatusBarHeight(isLandscape) + collapsibleOptions.headerStyle.height;
    collapsibleOptions.tabBarOptions = {
      ...collapsibleOptions.tabBarOptions,
      style: {
        ...tabStyle,
...
const getCollapsibleHeaderHeight = navigation => {
  let height =
    navigation.state.params && navigation.state.params.headerHeight
      ? navigation.state.params.headerHeight
      : 0;
...
export const withCollapsible = (WrappedScreen, collapsibleParams = {}) => {
  class _withCollapsible extends Component {
    constructor(props) {
      ...
      safeBounceHeight = collapsibleParams.safeBounceHeight || safeBounceHeight;
      ...
      if (!collapsibleParams.extraHeader) {
        ...
        const headerHeight =
          (collapsibleParams.extraHeaderStyle &&
            collapsibleParams.extraHeaderStyle.height) ||
          0;
        this.props.navigation.setParams({
          headerY: Animated.diffClamp(
            this.scrollY,
            0,
            safeBounceHeight + headerHeight
          ),
      ...
      const props = {
        ...
        collapsible: {
          paddingHeight: !collapsibleParams.extraHeader
            ? getCollapsibleHeaderHeight(navigation) +
              getCollapsibleTabHeight(navigation)
            : collapsibleParams.extraHeaderStyle.height,
        ...
        <View style={{ flex: 1 }}>
          ...
          {!collapsibleParams.extraHeader ? (
            <CollapsibleHeaderBackView
              iOSCollapsedColor={collapsibleParams.iOSCollapsedColor}
              navigation={navigation}
            />
          ) : (
            <CollapsibleExtraHeader
              navigation={navigation}
              style={collapsibleParams.extraHeaderStyle}
            >
